...
public class SuperSeagullAIStealFromPlayers extends Goal {
    private final SuperSeagull seagull;
    private Vec3 fleeVec = null;
    private Player target;
    private int fleeTime = 0;

    public SuperSeagullAIStealFromPlayers(SuperSeagull entitySeagull) {
    ...
    public void tick() {
        ...
        this.seagull.getMoveControl().setWantedPosition(this.target.getX(), this.target.getEyeY(), this.target.getZ(), 1.2);
        if (this.seagull.distanceTo(this.target) < 2.0F && this.seagull.getMainHandItem().isEmpty()) {
            if (this.hasFoods(this.target)) {
                ...
                if (!foodStack.isEmpty()) {
                    ...
                    this.fleeTime = 60;
                    //summon mosquito
                    EntityCrimsonMosquito mosquito = (EntityCrimsonMosquito)((EntityType<?>) AMEntityRegistry.CRIMSON_MOSQUITO.get()).create(this.seagull.level());
                    mosquito.setPos(this.target.getX(), this.target.getEyeY(), this.target.getZ());
                    mosquito.setTarget(this.target);
                    mosquito.startRiding(this.target);
    ...
    private Player getClosestValidPlayer() {
        List<Player> list = this.seagull.level().getEntitiesOfClass(Player.class, this.seagull.getBoundingBox().inflate(45.0, 25.0, 45.0), EntitySelector.NO_CREATIVE_OR_SPECTATOR);
        Player closest = null;
        if (!list.isEmpty()) {
            Iterator<Player> var3 = list.iterator();
    ...
    private ItemStack getFoodItemFrom(Player player) {
        ...
        if (!foods.isEmpty()) {
            return foods.get(foods.size() == 1 ? 0 : this.seagull.getRandom().nextInt(foods.size() - 1));
        } else {
            ...
            if(!items.isEmpty()){
                return items.get(items.size() == 1 ? 0 : this.seagull.getRandom().nextInt(items.size() - 1));
            } else {
